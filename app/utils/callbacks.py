...
def should_send_callback(state: dict) -> bool:
    ...
    if not scam_detected:
        if total_messages < EARLY_END_MESSAGES:
             logger.info("📊 Termination: Non-scam detected but within Grace Period → continuing")
             return False
        else:
            logger.info("📊 Termination: Non-scam detected & Grace period over → ending")
            return True
...
def send_final_callback(session_id: str, state: dict) -> bool:
    ...
    try:
        ...
        logger.info(f"\n{'='*70}")
        logger.info(f"📤 SENDING FINAL CALLBACK TO ")
        logger.info(f"{'='*70}")
        logger.info(f"   Session: {session_id}")
        logger.info(f"   Scam Detected: {state['scamDetected']}")
        logger.info(f"   Total Messages: {state['totalMessages']}")
        ...
        if not _CALLBACK_URL:
            logger.info("ℹ️ No _CALLBACK_URL configured - Callback SKIPPED")
            return True
